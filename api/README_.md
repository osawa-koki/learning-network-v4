...
```bash
learning-network-v4$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
learning-network-v4$ sam local start-api
learning-network-v4$ curl http://localhost:3000/
```
...
```bash
learning-network-v4$ sam logs -n HelloWorldFunction --stack-name learning-network-v4 --tail
```
...
```bash
learning-network-v4$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name learning-network-v4
```
